...
class VptTransformerEncoderLayer(BaseModule):
    ...
    def __init__(self,
                 ...
                 batch_first=True):
        super(VptTransformerEncoderLayer, self).__init__()
...
class VisionTransformer(BaseModule):
    ...
    def __init__(self,
        ...
        self.Prompt_Tokens = nn.Parameter(torch.zeros(num_layers, prompt_num, embed_dims))
        self.Prompt_Drop = nn.Dropout(p=prompt_drop)
    ...
    def forward(self, inputs):
        ...
        for i, layer in enumerate(self.layers):
            prompt = self.Prompt_Drop(self.Prompt_Tokens[i].unsqueeze(0).expand(B, -1, -1))
            x = layer(x, prompt)
